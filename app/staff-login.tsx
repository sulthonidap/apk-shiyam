...
export default function StaffLoginScreen() {
  ...
  const handleContinue = async () => {
    if (!name.trim() || !phone.trim() || !from.trim()) {
    ...
    try {
      ...
      const response = await fetch('https://api-shiyam.giescare.com/api/users', {
        ...
        body: JSON.stringify({ name, telephone: phone, address: from, role: 'staff' }),
      });
      if (!response.ok) {
      ...
      await AsyncStorage.setItem('userFrom', user.address || '');
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Login Staf</Text>
      ...
      <View style={styles.form}>
        ...
        <View style={styles.inputGroup}>
          <Text style={styles.label}>Asal Instansi</Text>
          <TextInput
            style={styles.input}
            value={from}
            onChangeText={setFrom}
            placeholder="Contoh: Klinik, Rumah Sakit"
            placeholderTextColor="#9CA3AF"
          />
        </View>
